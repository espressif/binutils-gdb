...
esp32ulp_pltpc_reloc (bfd * abfd ATTRIBUTE_UNUSED,
...
static bfd_reloc_status_type
esp32ulp_jumprelr_reloc (bfd * abfd, arelent * reloc_entry, asymbol * symbol,
			 ...
			 char **error_message ATTRIBUTE_UNUSED)
{
  ...
  if (bfd_is_und_section (symbol->section)
      && (symbol->flags & BSF_WEAK) == 0 && !relocatable)
  ...
  if ((symbol->name
       && symbol->section->name
       && !strcmp (symbol->name, symbol->section->name)) || !relocatable)
    {
...
static bfd_reloc_status_type
esp32s2ulp_jumprelr_reloc (bfd * abfd, arelent * reloc_entry,
...
static bfd_reloc_status_type
esp32ulp_bfd_reloc (bfd * abfd, arelent * reloc_entry, asymbol * symbol,
			...
			arelent * cache_ptr, Elf_Internal_Rela * dst)
{
  ...
  else if (r_type >= ESP32ULP_GNUEXT_RELOC_MIN
	   && r_type <= ESP32ULP_GNUEXT_RELOC_MAX)
    cache_ptr->howto =
      &esp32ulp_gnuext_howto_table[r_type - ESP32ULP_GNUEXT_RELOC_MIN];
				...
				const char *r_name)
{
  ...
  for (i = 0;
       i < (sizeof (esp32ulp_howto_table)
	    / sizeof (esp32ulp_howto_table[0])); i++)
  ...
  for (i = 0;
       i < (sizeof (esp32ulp_gnuext_howto_table)
	    / sizeof (esp32ulp_gnuext_howto_table[0])); i++)
...
static reloc_howto_type *
esp32ulp_reloc_type_lookup (bfd * abfd ATTRIBUTE_UNUSED, unsigned int r_type)
{
  if (r_type <= ESP32ULP_RELOC_MAX)
    return &esp32ulp_howto_table[r_type];
...
//bool elf32_esp32ulp_code_in_l1 = 0;
//
///* Set by ld emulation if --data-in-l1.  */
//bool elf32_esp32ulp_data_in_l1 = 0;
...
elf32_esp32ulp_reloc_type_class (const struct bfd_link_info *info
				 ATTRIBUTE_UNUSED,
...
static bfd_reloc_status_type
esp32ulp_final_link_relocate (Elf_Internal_Rela * rel,
			   ...
			   asection ** local_sections)
{
  ...
  for (; rel < relend; rel++, i++)
	...
	{
	  bool warned, ignored;
	  ESP32ULP_RELOC_FOR_GLOBAL_SYMBOL (info, input_bfd, input_section,
					    rel, r_symndx, symtab_hdr,
					    sym_hashes, h, sec, relocation,
...

	  {
		...
		if ((off & 1) != 0)
		...
		else
		  {
			...
			outrel.r_info = ELF32_R_INFO (0, R_ESP32ULP_RIMM16);
	...
	do_default:
	  ...
	  rel->r_addend = 0;
	  r =
	    esp32ulp_final_link_relocate (rel, howto, input_bfd,
					  input_section, contents, address,
				...
				enum insert_option insert)
{
  ...
  loc =
    (struct esp32ulpfdpic_relocs_info **) htab_find_slot (ht, entry, insert);
...
inline static void
esp32ulpfdpic_pic_merge_early_relocs_info (struct esp32ulpfdpic_relocs_info
					   *e2,
					   struct esp32ulpfdpic_relocs_info
					   const *e1)
{
...
static bfd_vma
_esp32ulpfdpic_add_rofixup (bfd * output_bfd, asection * rofixup,
			    bfd_vma offset,
			    struct esp32ulpfdpic_relocs_info *entry)
{
				...
				asection ** local_sections)
{
  ...
  Elf_Internal_Rela *rel;
  Elf_Internal_Rela *relend;
  unsigned isec_segment, check_segment[2];
  ...
  if (bed->want_got_sym)
    {
      ...
      h =
	_bfd_elf_define_linkage_sym (abfd, info, s, "__GLOBAL_OFFSET_TABLE_");
...
static bool
elf32_esp32ulpfdpic_create_dynamic_sections (bfd * abfd,
					     struct bfd_link_info *info)
{
  ...
  BFD_ASSERT (esp32ulpfdpic_got_section (info)
	      && esp32ulpfdpic_gotrel_section (info)
				     ...
				     *dinfo)
{
  ...
  entry->plt = entry->call
    && entry->symndx == -1
    && !ESP32ULPFDPIC_SYM_LOCAL (dinfo->info, entry->d.h)
...
_esp32ulpfdpic_compute_got_alloc_data (struct
				       _esp32ulpfdpic_dynamic_got_alloc_data
...
inline static bfd_signed_vma
_esp32ulpfdpic_get_got_entry (struct _esp32ulpfdpic_dynamic_got_alloc_data
			      *gad)
{
  bfd_signed_vma ret;
...
inline static bfd_signed_vma
_esp32ulpfdpic_get_fd_entry (struct _esp32ulpfdpic_dynamic_got_alloc_data
			     *gad)
{
...
static int
_esp32ulpfdpic_reset_got_plt_entries (void **entryp,
...
_esp32ulpfdpic_resolve_final_relocs_info (void **entryp, void *p)
{
  ...
  if (entry->symndx == -1)
	...
	return 1;
      ...
      oentry =
	esp32ulpfdpic_relocs_info_for_global (*htab, 0, h, entry->addend,
					      NO_INSERT);
			     ...
			     *gpinfop)
{
  ...
  odd = _esp32ulpfdpic_compute_got_alloc_data (&gpinfop->gothilo,
					       ...
					       gpinfop->g.fdhilo,
					       gpinfop->g.fdplt -
					       gpinfop->got17m4.fdplt,
					       (bfd_vma) 1 << (32 - 1));

  /* Now assign (most) GOT offsets.  */
  htab_traverse (esp32ulpfdpic_relocs_info (info),
		 _esp32ulpfdpic_assign_got_entries, gpinfop);
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
    esp32ulpfdpic_pltrel_section (info)->size =
      gpinfop->g.lzplt / LZPLT_NORMAL_SIZE *
      get_elf_backend_data (output_bfd)->s->sizeof_rel;
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
	...
	+
	((gpinfop->g.lzplt + (ESP32ULPFDPIC_LZPLT_BLOCK_SIZE - 4) -
	  LZPLT_NORMAL_SIZE) / (ESP32ULPFDPIC_LZPLT_BLOCK_SIZE -
  ...
  htab_traverse (esp32ulpfdpic_relocs_info (info),
		 _esp32ulpfdpic_assign_plt_entries, gpinfop);
  ...
  for (;;)
    {
      ...
      htab_traverse (relocs, _esp32ulpfdpic_resolve_final_relocs_info,
		     &relocs);
...
esp32ulpfdpic_elf_discard_info (bfd * ibfd,
				...
				ATTRIBUTE_UNUSED, struct bfd_link_info *info)
{
  bool changed = false;
  asection *s;
  bfd *obfd = NULL;
  ...
  if (changed)
    {
      ...
      htab_traverse (esp32ulpfdpic_relocs_info (info),
		     _esp32ulpfdpic_reset_got_plt_entries, NULL);
					     ...
					     struct bfd_link_info *info)
{
  ...
  if (esp32ulpfdpic_got_section (info))
      ...
      if (esp32ulpfdpic_gotfixup_section (info))
	{
	  ...
	  _esp32ulpfdpic_add_rofixup (output_bfd,
				      esp32ulpfdpic_gotfixup_section (info),
				      got_value, 0);
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      for (; dyncon < dynconend; dyncon++)
	{
	  ...
	  switch (dyn.d_tag)
	    ...
	    case DT_PLTGOT:
	      dyn.d_un.d_ptr =
		esp32ulpfdpic_got_section (info)->output_section->vma +
	    ...
	    case DT_JMPREL:
	      dyn.d_un.d_ptr =
		esp32ulpfdpic_pltrel_section (info)->output_section->vma +
				     ...
				     bfd_vma * encoded)
{
  ...
  if (!h || (_esp32ulpfdpic_osec_to_segment (abfd, osec)
	     == _esp32ulpfdpic_osec_to_segment (abfd,
						loc_sec->output_section)))
  ...
  if ((h->root.u.def.section->flags & SEC_ALLOC) != 0)
    {
      _bfd_error_handler (_
			  ("the esp32ulp target does not currently support the generation of copy relocations"));
      return false;
    }
#endif
