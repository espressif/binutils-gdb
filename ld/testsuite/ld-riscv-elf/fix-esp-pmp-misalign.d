#name: Espressif PMP misalign linker insert
...
[ 	]+[0-9a-f]+:[ 	]+00a62023[ 	]+sw[ 	]+a0,0\(a2\)
# store through the load's own base cannot leave the load's PMP region
[0-9a-f]+ <same_base>:
[ 	]+[0-9a-f]+:[ 	]+0005a503[ 	]+lw[ 	]+a0,0\(a1\)
[ 	]+[0-9a-f]+:[ 	]+00a5a023[ 	]+sw[ 	]+a0,0\(a1\)
# a byte load never crosses a 4-byte boundary, so it arms nothing
[0-9a-f]+ <byte_load>:
[ 	]+[0-9a-f]+:[ 	]+00058503[ 	]+lb[ 	]+a0,0\(a1\)
[ 	]+[0-9a-f]+:[ 	]+00a62023[ 	]+sw[ 	]+a0,0\(a2\)
# the store's own width is irrelevant, so a byte store is padded too
[0-9a-f]+ <byte_store>:
[ 	]+[0-9a-f]+:[ 	]+0005a503[ 	]+lw[ 	]+a0,0\(a1\)
[ 	]+[0-9a-f]+:[ 	]+00000013[ 	]+nop
[ 	]+[0-9a-f]+:[ 	]+00000013[ 	]+nop
[ 	]+[0-9a-f]+:[ 	]+00a60023[ 	]+sb[ 	]+a0,0\(a2\)
# the branch separates the load from the store on its taken path
...
[0-9a-f]+ <btarget>:
[ 	]+[0-9a-f]+:[ 	]+00a62023[ 	]+sw[ 	]+a0,0\(a2\)
# only the second load is followed by the store
[0-9a-f]+ <load_between>:
[ 	]+[0-9a-f]+:[ 	]+0005a503[ 	]+lw[ 	]+a0,0\(a1\)
