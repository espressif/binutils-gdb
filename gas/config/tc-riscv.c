...
md_apply_fix (fixS *fixP, valueT *valP, segT seg)
{
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_RISCV_ESP_LP_OFFSET_9_OLD:
      if (fixP->fx_addsy)
	{
	  /* Fill in a tentative value to improve objdump readability.  */
	  bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
    ...
    case BFD_RELOC_RISCV_ESP_LP_OFFSET_12_OLD:
      if (fixP->fx_addsy)
	{
	  /* Fill in a tentative value to improve objdump readability.  */
	  bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
    ...
    case BFD_RELOC_RISCV_ESP_LP_OFFSET_9_NEW:
	if (fixP->fx_addsy)
	  {
	    /* Fill in a tentative value to improve objdump readability.  */
	    bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
    ...
    case BFD_RELOC_RISCV_ESP_LP_OFFSET_12_NEW:
	if (fixP->fx_addsy)
	  {
	    /* Fill in a tentative value to improve objdump readability.  */
	    bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
