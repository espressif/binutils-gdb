  ...
  for (oparg = opc->args; *oparg; ++oparg)
	...
	case 'G': /* RV_XESPPIE */
	  switch (*++oparg)
		...
		case '1': used_bits |= ENCODE_ESPPIE_RD_1 (-1U); break;	/* Gd1 */
		default:	/* Gd[.] */
		  goto unknown_validate_operand;
		}
	      break;
		...
		case '1': used_bits |= ENCODE_ESPPIE_RS2_1 (-1U); break;	/* Gt1 */
		default:	/* Gt[.] */
		  goto unknown_validate_operand;
		}
	      break;
      ...
      for (oparg = insn->args;; ++oparg)
	{
	  ...
	  switch (*oparg)
			...
			case '4':
			  switch (*++oparg)
			    ...
			    case '0':	/* Go440 */
				  ...
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_16_16_0 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '1':	/* Go441 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_16_16_1 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '2':	/* Go442 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_16_16_2 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			...
			case '4':
			  switch (*++oparg)
			    ...
			    case '0':	/* Go840 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_16_0 ((valueT)
								    imm_expr->
								    X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '1':	/* Go841 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_16_1 ((valueT)
								    imm_expr->
								    X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '2':	/* Go842 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_16_2 ((valueT)
								    imm_expr->
								    X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '3':	/* Go843 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_16_3 ((valueT)
								    imm_expr->
								    X_add_number))
				break;
			      ip->insn_opcode |=
			...
			case '2':
			  switch (*++oparg)
			    ...
			    case '0':	/* Go820 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_4_0 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '1':	/* Go821 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_4_1 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			...
			case '3':
			  switch (*++oparg)
			    ...
			    case '0':	/* Go830 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_8_0 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '1':	/* Go831 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_8_1 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
			    ...
			    case '2':	/* Go832 */
				  ...
				  (imm_expr, imm_reloc, asarg, p)
				  || imm_expr->X_op != O_constant
				  || !VALID_ESPPIE_OFFSET_256_8_2 ((valueT)
								   imm_expr->
								   X_add_number))
				break;
			      ip->insn_opcode |=
		  ...
		  switch (*++oparg)
		    ...
		    case '1':	/* Gd1 */
			  ...
			  || !((regno >= 8 && regno <= 15)
			       || (regno >= 24 && regno <= 31)))
			break;
		      INSERT_OPERAND (ESPPIE_RD_1, *ip, regno);
		      continue;
		    default:	/* Gd[.] */
		      goto unknown_riscv_ip_operand;
		    }
		  ...
		  switch (*++oparg)
		    ...
		    case '0':	/* Gt0 */
			  ...
			  || !((regno >= 8 && regno <= 15)
			       || (regno >= 24 && regno <= 31)))
			break;
		      INSERT_OPERAND (ESPPIE_RS2_0, *ip, regno);
		      continue;
		    case '1':	/* Gt1 */
		      if (!reg_lookup (&asarg, RCLASS_GPR, &regno)
			  || !((regno >= 8 && regno <= 15)
			       || (regno >= 24 && regno <= 31)))
			break;
		      ip->insn_opcode |= ENCODE_ESPPIE_RS2_1 (regno);
		      continue;
		    default:	/* Gt[.] */
		      goto unknown_riscv_ip_operand;
		    }
		  ...
		  switch (*++oparg)
			...
			case '0':	/* Gx40 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_16_0 ((valueT)
							    imm_expr->
							    X_add_number))
			    break;
			  ip->insn_opcode |=
			...
			case '1':	/* Gx41 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_16_1 ((valueT)
							    imm_expr->
							    X_add_number))
			    break;
			  ip->insn_opcode |=
			...
			case '2':	/* Gx42 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_16_2 ((valueT)
							    imm_expr->
							    X_add_number))
			    break;
			  ip->insn_opcode |=
			...
			case '3':	/* Gx43 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_16_3 ((valueT)
							    imm_expr->
							    X_add_number))
			    break;
			  ip->insn_opcode |=
			...
			case '4':	/* Gx44 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_16_4 ((valueT)
							    imm_expr->
							    X_add_number))
			    break;
			  ip->insn_opcode |=
			...
			case '0':	/* Gx10 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_2_0 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_2_0 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '1':	/* Gx11 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_2_1 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_2_1 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '2':	/* Gx12 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_2_2 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_2_2 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '3':	/* Gx13 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_2_3 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_2_3 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '4':	/* Gx14 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_2_4 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_2_4 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '5':	/* Gx15 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_2_5 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_2_5 (imm_expr->X_add_number);
			  goto esp_imm_done;
			default:	/* Gx1[.] */
			  goto unknown_riscv_ip_operand;
			}
			...
			case '0':	/* Gx20 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_0 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_0 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '1':	/* Gx21 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_1 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_1 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '2':	/* Gx22 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_2 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_2 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '3':	/* Gx23 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_3 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_3 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '4':	/* Gx24 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_4 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_4 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '5':	/* Gx25 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_5 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_5 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '6':	/* Gx26 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_4_6 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_4_6 (imm_expr->X_add_number);
			  goto esp_imm_done;
			default:	/* Gx2[.] */
			  goto unknown_riscv_ip_operand;
			}
			...
			case '0':	/* Gx30 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_8_0 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_8_0 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '1':	/* Gx31 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_8_1 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_8_1 (imm_expr->X_add_number);
			  goto esp_imm_done;
			case '2':	/* Gx32 */
			  if (my_getSmallExpression
			      (imm_expr, imm_reloc, asarg, p)
			      || imm_expr->X_op != O_constant
			      || !VALID_ESPPIE_SELECT_8_2 ((valueT) imm_expr->
							   X_add_number))
			    break;
			  ip->insn_opcode |=
			    ENCODE_ESPPIE_SELECT_8_2 (imm_expr->X_add_number);
			  goto esp_imm_done;
			default:	/* Gx3[.] */
			  goto unknown_riscv_ip_operand;
			}
		...
		case 'u':	/* Gu */
		  if (my_getSmallExpression (imm_expr, imm_reloc, asarg, p)
		      || imm_expr->X_op != O_constant
		      || !VALID_ESPPIE_UPD_4 ((valueT) imm_expr->
					      X_add_number))
		    break;
		  ip->insn_opcode |=
