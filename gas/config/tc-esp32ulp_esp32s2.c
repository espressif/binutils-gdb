...
INSTR_T
esp32ulp_gen_wait_esp32s2 (void)
{
...
INSTR_T
esp32ulp_gen_alur_esp32s2 (int dst, int src1, int src2, int operation)
{
...
INSTR_T
esp32ulp_gen_alui_esp32s2 (int dst, int src1, Expr_Node * addr, int operation)
{
...
INSTR_T
esp32ulp_cmd_stage_esp32s2 (int dir, Expr_Node * imm)
{
  int imm_val = EXPR_VALUE (imm);
  unsigned int local_op = I_ALUS (dir, imm_val);

  return conscode (gencode (local_op),
		   Expr_Node_Gen_Reloc (imm, BFD_RELOC_ESP32S2ULP_STAGE));
}

INSTR_T
esp32ulp_cmd_stage_rst_esp32s2 ()
{
...
INSTR_T
esp32ulp_gen_jump_r_esp32s2 (int dst_reg, int cond)
{
...
INSTR_T
esp32ulp_gen_jump_i_esp32s2 (Expr_Node * addr, int cond)
{
  int val;
  int rel = 0;
  ...
  if ((val % 4) != 0)
    error ("%s",
	   "Esp32S2Ulp is 32-bit addressing processor. Relative jump value must be divided by 4.");
  unsigned int local_op = I_JUMP_RI (0, val >> 2, cond, 0);
  return conscode (gencode (local_op), Expr_Node_Gen_Reloc (addr, rel));
}

INSTR_T
esp32ulp_cmd_reg_rd_esp32s2 (Expr_Node * addr, Expr_Node * high,
			     Expr_Node * low)
{
  ...
  if (addr_val > DR_REG_MAX_DIRECT)
    {
      if ((addr_val >= DR_REG_RTCCNTL_BASE)
	  && (addr_val < DR_REG_IO_MUX_BASE))
	{
  ...
  return conscode (gencode (local_op),
		   conctcode (Expr_Node_Gen_Reloc
			      (addr, BFD_RELOC_ESP32S2ULP_REG_RW_ADDR),
			      conctcode (Expr_Node_Gen_Reloc
					 (high,
					  BFD_RELOC_ESP32S2ULP_REG_RW_HIGH),
					 Expr_Node_Gen_Reloc (low,
							      BFD_RELOC_ESP32S2ULP_REG_RW_LOW))));
}

INSTR_T
esp32ulp_cmd_reg_wr_esp32s2 (Expr_Node * addr, Expr_Node * high,
			     Expr_Node * low, Expr_Node * data)
{
  unsigned int addr_val = EXPR_VALUE (addr);
  unsigned int high_val = EXPR_VALUE (high);
  unsigned int low_val = EXPR_VALUE (low);
...
INSTR_T
esp32ulp_cmd_i2c_rd_esp32s2 (Expr_Node * i2c_addr, Expr_Node * high,
			     Expr_Node * low, Expr_Node * i2c_sel)
{
  ...
  unsigned int local_op =
    I_I2C_RW (addr_val, 0, low_val, high_val, i2c_sel_val, OPCODE_I2C_RD);

  return conscode (gencode (local_op),
		   conctcode (Expr_Node_Gen_Reloc
			      (i2c_addr, BFD_RELOC_ESP32S2ULP_I2C_RW_ADDR),
			      conctcode (Expr_Node_Gen_Reloc
					 (i2c_sel,
...
INSTR_T
esp32ulp_cmd_i2c_wr_esp32s2 (Expr_Node * i2c_addr, Expr_Node * high,
			     Expr_Node * low, Expr_Node * i2c_sel,
			     Expr_Node * data)
{
  unsigned int addr_val = EXPR_VALUE (i2c_addr);
  ...
  unsigned int local_op =
    I_I2C_RW (addr_val, data_val, low_val, high_val, i2c_sel_val,
	      OPCODE_I2C_WR);
...
static int
ConvertBrcond2JumpRs2 (int cond)
{
  int result = JUMPR_EQ;
  switch (cond)
    {
...
static int
ConvertBrcond2JumpSs2 (int cond)
{
  int result = JUMPS_EQ;
  switch (cond)
    {
...
INSTR_T
esp32ulp_cmd_jump_relr_esp32s2 (Expr_Node * step, Expr_Node * thresh,
				int cond)
{
  ...
  if ((cond == JUMPR_LE) || (cond == JUMPR_GE))	// EQ == 3, GT = 4
      ...
      if (cond == JUMPR_GE)	// GT
	{
      ...
      unsigned int local_walk =
	I_JUMP_RELR (thresh_val, walk_cond, step_val >> 2);
      if (step_val < 0)
	step_val = step_val - 4;
      unsigned int local_result =
	I_JUMP_RELR (thresh_val, result_cond, step_val >> 2);

      INSTR_T result = conscode (gencode (local_result),
				 conctcode (Expr_Node_Gen_Reloc (step,
								 BFD_RELOC_ESP32S2ULP_JUMPR_STEP),
					    Expr_Node_Gen_Reloc (thresh,
  ...
  else
    {
      ...
      return conscode (gencode (local_op),
		       conctcode (Expr_Node_Gen_Reloc
				  (step, BFD_RELOC_ESP32S2ULP_JUMPR_STEP),
				  Expr_Node_Gen_Reloc (thresh,
						       BFD_RELOC_ESP32S2ULP_JUMPR_THRESH)));
    }
}
...
INSTR_T
esp32ulp_cmd_jump_rels_esp32s2 (Expr_Node * step, Expr_Node * thresh,
				int cond)
{
...
INSTR_T
esp32ulp_rd_mem_addr_esp32s2 (int lh, int dst_reg, int src_reg,
			      Expr_Node * addr)
{
...
INSTR_T
esp32ulp_wr_mem_stlh_esp32s2 (int low_hi, int dst_reg, int src_reg,
			      Expr_Node * offset)
{
  int addr_val = EXPR_VALUE (offset);
  unsigned int local_op =
    WR_MEM (dst_reg, src_reg, addr_val >> 2, 0, low_hi, 3, 0x4);
...
INSTR_T
esp32ulp_wr_mem_st32_esp32s2 (int dst_reg, int src_reg, Expr_Node * offset,
			      Expr_Node * label)
{
  unsigned int label_val = EXPR_VALUE (label);
  unsigned int offset_val = EXPR_VALUE (offset);
  unsigned int local_op =
    WR_MEM (dst_reg, src_reg, offset_val >> 2, label_val, 0, 0, 0x4);

  return conscode (gencode (local_op),
		   conctcode (Expr_Node_Gen_Reloc
			      (offset, BFD_RELOC_ESP32S2ULP_WR_MEM),
			      Expr_Node_Gen_Reloc (label,
						   BFD_RELOC_ESP32S2ULP_WR_MEM_LABEL)));
}

INSTR_T
esp32ulp_wr_mem_sti32_esp32s2 (int dst_reg, int src_reg, Expr_Node * label)
{
  unsigned int label_val = EXPR_VALUE (label);
...
INSTR_T
esp32ulp_wr_mem_st_l_esp32s2 (int low_hi, int dst_reg, int src_reg,
			      Expr_Node * offset, Expr_Node * label)
{
  unsigned int label_val = EXPR_VALUE (label);
  unsigned int offset_val = EXPR_VALUE (offset);
  unsigned int local_op =
    WR_MEM (dst_reg, src_reg, offset_val >> 2, label_val, low_hi, 1, 0x4);

  return conscode (gencode (local_op),
		   conctcode (Expr_Node_Gen_Reloc
			      (label, BFD_RELOC_ESP32S2ULP_WR_MEM_LABEL),
			      Expr_Node_Gen_Reloc (offset,
						   BFD_RELOC_ESP32S2ULP_WR_MEM)));
}

INSTR_T
esp32ulp_wr_mem_sti_l_esp32s2 (int dst_reg, int src_reg, Expr_Node * label)
{
  unsigned int label_val = EXPR_VALUE (label);
...
INSTR_T
esp32ulp_wr_mem_sti_esp32s2 (int dst_reg, int src_reg)
{
...
void
md_apply_fix_esp32s2 (fixS * fixP, valueT * valueP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_JUMPR"));
      if (fixP->fx_addsy != NULL)	// relocation will be done not in linker
	{
	  asymbol *sym = symbol_get_bfdsym (fixP->fx_addsy);
	  int force_reloc = S_FORCE_RELOC (fixP->fx_addsy, 1);
    ...
    case BFD_RELOC_ESP32S2ULP_WR_MEM_LABEL:
      if ((value > 3) || (value < 0))
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_WR_MEM_LABEL"));
      temp_val = 0;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0x3 << 4));
      temp_val |= (value << 4);
    ...
    case BFD_RELOC_ESP32S2ULP_JUMPR_STEP:
      if (value < -0x8000 || value > 0x7fff)
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_JUMPR_STEP"));
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_JUMPR_THRESH"));
      temp_val = 0;
      value &= 0xffff;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0xffff << 0));
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_JUMPS_THRESH"));
      temp_val = 0;
      value &= 0xff;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0xff << 0));
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_REG_RW_HIGH"));
      temp_val = 0;
      value &= 0x1f;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0x1f << 23));
    ...
    case BFD_RELOC_ESP32S2ULP_REG_RW_LOW:
      if ((value > 0x1f) || (value < 0))
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_REG_RW_LOW"));
      temp_val = 0;
      value &= 0x1f;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0x1f << 18));
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_I2C_RW_HIGH"));
      temp_val = 0;
      value &= 0x7;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0x7 << 19));
    ...
    case BFD_RELOC_ESP32S2ULP_I2C_RW_LOW:
      if ((value > 0x7) || (value < -0x7))
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_I2C_RW_LOW"));
      temp_val = 0;
      value &= 0x7;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0x7 << 16));
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_I2C_RW_SEL"));
      temp_val = 0;
      value &= 0xf;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0xf << 22));
	...
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_I2C_RW_ADDR"));
      temp_val = 0;
      value &= 0xff;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0xff << 0));
    ...
    case BFD_RELOC_ESP32S2ULP_I2C_RW_DATA:
      if ((value > 0xff) || (value < -0xff))
	as_bad_where (fixP->fx_file, fixP->fx_line,
		      _("rel too far BFD_RELOC_ESP32S2ULP_I2C_RW_DATA"));
      temp_val = 0;
      value &= 0xff;
      memcpy (&temp_val, where, 4);
      temp_val &= ~((0xff << 8));
    ...
    default:
      fprintf (stderr,
	       "Relocation %d not handled in Esp32s2 gas."
	       " Contact support.\n", fixP->fx_r_type);
      return;
    }
...
static INSTR_T
Expr_Node_Gen_Reloc_R_s2 (Expr_Node * head)
{
  ...
  switch (head->type)
    ...
    case Expr_Node_Constant:
      note =
	conscode (note_reloc2
		  (gencode (0), con, BFD_ARELOC_ESP32ULP_CONST,
		   head->value.i_value, 0), NULL_CODE);
      break;
    case Expr_Node_Reloc:
      note =
	conscode (note_reloc (gencode (0), head, BFD_ARELOC_ESP32ULP_PUSH, 0),
		  NULL_CODE);
      break;
    case Expr_Node_Unop:
      note1 =
	conscode (Expr_Node_Gen_Reloc_R_s2 (head->Left_Child), NULL_CODE);
      switch (head->value.op_value)
	{
	case Expr_Op_Type_NEG:
	  note =
	    conctcode (note1,
		       conscode (note_reloc1
				 (gencode (0), op, BFD_ARELOC_ESP32ULP_NEG,
				  0), NULL_CODE));
	  break;
	case Expr_Op_Type_COMP:
	  note =
	    conctcode (note1,
		       conscode (note_reloc1
				 (gencode (0), op, BFD_ARELOC_ESP32ULP_COMP,
				  0), NULL_CODE));
	  break;
	default:
	  fprintf (stderr,
		   "%s:%d:Unknown operator found for arithmetic"
		   " relocation", __FILE__, __LINE__);
	}
      break;
    default:
      fprintf (stderr,
	       "%s:%d:Unknown node expression found during "
...
INSTR_T
Expr_Node_Gen_Reloc_esp32s2 (Expr_Node * head, int parent_reloc)
{
  ...
  if (head->type == Expr_Node_Constant)
    ...
    {
      note =
	note_reloc1 (gencode (0), head->value.s_value, parent_reloc, pcrel);
      if (note1 != NULL_CODE)
	...
	val = -val;
      note =
	conscode (note_reloc2
		  (gencode (0), head->Left_Child->value.s_value, parent_reloc,
		   val, 0), NULL_CODE);
      if (note1 != NULL_CODE)
	note = conscode (note1, note);
    }
