...
const char wrong_cpu_message[] =
  "This command not supported by this version of ULP CPU. Please use different --mcpu=... option to compile.";

void
md_show_usage (FILE * stream)
{
  fprintf (stream,
	   _(" ESP32 ULP processor has no specific assembler options:\n"));
}
...
md_assemble (char *line)
{
  ...
  while (insn)
      ...
      if (insn->reloc && insn->exp->symbol)
	{
	  ...
	  fix_new (frag_now, toP - frag_now->fr_literal, insn_size,
		   insn->exp->symbol, insn->exp->value, insn->pcrel,
		   insn->reloc);
	}
#ifdef DEBUG
...
INSTR_T
note_reloc2 (INSTR_T code, const char *symbol, int reloc, int value,
	     int pcrel)
{
  code->reloc = reloc;
...
Expr_Node *
Expr_Node_Create (Expr_Node_Type type, Expr_Node_Value value,
		  Expr_Node * Left_Child, Expr_Node * Right_Child)
{
...
INSTR_T
esp32ulp_gen_alur (int dst, int src1, int src2, int operation)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_gen_alui (int dst, int src1, Expr_Node * addr, int operation)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_gen_jump_r (int dst_reg, int cond)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_gen_jump_i (Expr_Node * addr, int cond)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_jump_relr (Expr_Node * step, Expr_Node * thresh, int cond)
{
...
INSTR_T
esp32ulp_cmd_jump_rels (Expr_Node * step, Expr_Node * thresh, int cond)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_reg_rd (Expr_Node * addr, Expr_Node * high, Expr_Node * low)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_reg_wr (Expr_Node * addr, Expr_Node * high, Expr_Node * low,
		     Expr_Node * data)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_i2c_rd (Expr_Node * i2c_addr, Expr_Node * high, Expr_Node * low,
		     Expr_Node * i2c_sel)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_i2c_wr (Expr_Node * i2c_addr, Expr_Node * high, Expr_Node * low,
		     Expr_Node * i2c_sel, Expr_Node * data)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_rd_mem_addr (int lh, int dst_reg, int src_reg, Expr_Node * addr)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_sleep (Expr_Node * cycles)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_wakeup (Expr_Node * wake)
{
  (void) wake;
...
INSTR_T
esp32ulp_cmd_stage (int dir, Expr_Node * imm)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_cmd_wait (Expr_Node * cycles)
{
...
INSTR_T
esp32ulp_cmd_tsens (int dreg, Expr_Node * delay)
{
...
INSTR_T
esp32ulp_cmd_adc (int dreg, Expr_Node * sar_sel, Expr_Node * mux)
{
  ...
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
      return conscode (gencode (local_op),
		       conctcode (Expr_Node_Gen_Reloc
				  (mux, BFD_RELOC_ESP32ULP_ADC_MUX),
				  Expr_Node_Gen_Reloc (sar_sel,
  ...
  else if (ulp_cpu_type == CPU_TYPE_ESP32ULP_S2)
    {
      return conscode (gencode (local_op),
		       conctcode (Expr_Node_Gen_Reloc
				  (mux, BFD_RELOC_ESP32S2ULP_ADC_MUX),
				  Expr_Node_Gen_Reloc (sar_sel,
...
INSTR_T
esp32ulp_wr_mem_stlh (int low_hi, int dst_reg, int src_reg,
		      Expr_Node * offset)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_wr_mem_st32 (int dst_reg, int src_reg, Expr_Node * offset,
		      Expr_Node * label)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
...
INSTR_T
esp32ulp_wr_mem_sti32 (int dst_reg, int src_reg, Expr_Node * label)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
      as_fatal (wrong_cpu_message);
...
INSTR_T
esp32ulp_wr_mem_st_l (int low_hi, int dst_reg, int src_reg,
		      Expr_Node * offset, Expr_Node * label)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    ...
    {
      return esp32ulp_wr_mem_st_l_esp32s2 (low_hi, dst_reg, src_reg, offset,
					   label);
    }
  return 0;
}
...
INSTR_T
esp32ulp_wr_mem_sti_l (int dst_reg, int src_reg, Expr_Node * label)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
      as_fatal (wrong_cpu_message);
...
INSTR_T
esp32ulp_wr_mem_sti (int dst_reg, int src_reg)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
      as_fatal (wrong_cpu_message);
...
INSTR_T
esp32ulp_wr_mem_sto (Expr_Node * offset)
{
  if (ulp_cpu_type == CPU_TYPE_ESP32ULP)
    {
      as_fatal (wrong_cpu_message);
